"""
Attack Success Evaluation for Qwen3-VL

Requires: pip install "transformers>=4.57.0"
...
# Gracefully handle older transformers that lack AutoModelForImageTextToText
try:
    ...
    try:
        ...
        _LOADER = Qwen3VLForConditionalGeneration
    except ImportError:
        raise ImportError(
            "Could not import a Qwen3-VL model class. "
...
OUT_CSV    = "<TO_INSERT>"
MODEL_ID   = "Qwen/Qwen3-VL-8B-Instruct"  # or 32B / 235B-A22B
SPLIT      = "train"
...
model = _LOADER.from_pretrained(
    MODEL_ID,
    torch_dtype=torch.bfloat16,  # Qwen3-VL ships in bf16
    device_map="auto",
...
def ask_yes_no(image_path: str, color: str, category: str) -> bool | None:
    ...
    # Qwen3-VL: apply_chat_template with tokenize=True returns the full input dict
    ...
    generated_ids_trimmed = [out[len(inp):] for inp, out in zip(inputs["input_ids"], output_ids)]
