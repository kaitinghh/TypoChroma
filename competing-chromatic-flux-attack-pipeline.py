...
COLOR_SIMILARITY_GROUPS: List[List[str]] = [
    ["red", "pink"],
    ["red", "orange"],
    ["orange", "brown"],
    ["yellow", "orange"],
...
def get_similar_colors(color: str) -> set:
    banned = {color}
...
def opposite_color(c: str) -> str:
    ...
    h = HUE_DEG.get(c)
    if h is None:     return "blue"
    target = (h + 180.0) % 360.0
    best, best_d = None, 1e9
    for cand, ch in HUE_DEG.items():
        if cand == c or ch is None: continue
        d = abs(((ch - target + 180) % 360) - 180)
        if d < best_d:
            best_d = d; best = cand
    return best or "blue"

def fallback_color(c: str) -> str:
    banned = get_similar_colors(c)
    opposite = opposite_color(c)
    if opposite not in banned:
        return opposite
    for candidate in ["blue", "green", "red", "yellow", "purple", "orange",
                      "pink", "brown", "black", "white", "gray"]:
        if candidate not in banned:
            return candidate
    return "blue"

def dominant_vicinity_color(image: Image.Image, mask_bool: np.ndarray, true_color: str) -> str:
    """
    Finds the largest contiguous same-color region outside the object mask,
    excluding colors perceptually similar to true_color.
    """
    img_arr = np.asarray(image.convert("RGB"), dtype=np.float32)
    H, W = mask_bool.shape

    color_names = list(COLOR_RGB.keys())
    color_refs  = np.array([COLOR_RGB[c] for c in color_names], dtype=np.float32)

    pixels_flat = img_arr.reshape(-1, 3)
    diffs  = pixels_flat[:, None, :] - color_refs[None, :, :]
    dists  = np.sqrt((diffs ** 2).sum(axis=-1))
    labels = np.argmin(dists, axis=1).reshape(H, W)

    banned = get_similar_colors(true_color)
    print(f"  [vicinity] true={true_color} banned={banned}")

    largest_size = 0
    largest_color = None

    for color_idx, color_name in enumerate(color_names):
        if color_name in banned:
            continue
        color_region = (labels == color_idx) & ~mask_bool
        labeled_arr, num_features = ndimage.label(color_region)
        if num_features == 0:
            continue
        sizes = ndimage.sum(color_region, labeled_arr, range(1, num_features + 1))
        max_size = int(max(sizes))
        if max_size > largest_size:
            largest_size = max_size
            largest_color = color_name

    print(f"  [vicinity] largest_color={largest_color} size={largest_size}")

    if largest_color is None:
        print(f"  [vicinity] No valid color found — falling back")
        return fallback_color(true_color)
...
def main():
    ...
    missing = needed - set(df_full.columns)
    if missing:
        raise ValueError(f"CSV missing columns: {missing}")

    # Filter to eval ann_ids
    print(f"Loading eval CSV: {EVAL_CSV}")
    df_eval = pd.read_csv(EVAL_CSV)
    if "ann_id" not in df_eval.columns:
        df_eval["ann_id"] = (
            df_eval["attacked_path"]
            .str.extract(r"_ann(\d+)_")[0]
            .astype(int)
        )
    eval_ann_ids = set(df_eval["ann_id"].astype(int).tolist())
    df_full = df_full[df_full["ann_id"].astype(int).isin(eval_ann_ids)].reset_index(drop=True)
    print(f"  Eval ann_ids: {len(eval_ann_ids)} → matched rows: {len(df_full)}")

    df_full = df_full.sort_values("ann_id").reset_index(drop=True)
    df = df_full.copy()

    # Resumability — skip already-processed ann_ids
    if os.path.exists(OUT_LOG):
        df_done = pd.read_csv(OUT_LOG)
        done_ann_ids = set(df_done["ann_id"].astype(int).tolist())
        before = len(df)
        df = df[~df["ann_id"].astype(int).isin(done_ann_ids)].reset_index(drop=True)
        print(f"  Resuming: {len(done_ann_ids)} already done, {before - len(df)} skipped, {len(df)} remaining")
    else:
    ...
    print(f"\nProcessing {len(df)} rows | OUT_DIR={OUT_DIR} | OUT_LOG={OUT_LOG}\n")
    ...
    for idx, row in df.iterrows():
        ...
        # Seed on ann_id (not idx) for consistency across partitions
        base_seed = (SEED * 1000003 + ann_id * 9176) & 0xFFFFFFFF
        ...
        with open(OUT_LOG, "a", newline="", encoding="utf-8") as f:
            ...
            if write_header:
                writer.writeheader()
            writer.writerow({key: rows_out[-1].get(key, "") for key in CSV_HEADERS})

        print(f"  ✅ Logged to {OUT_LOG}")

    print(f"\n✅ Done.")
    print(f"   Images written : {OUT_DIR}")
